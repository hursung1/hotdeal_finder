...
├── crawler.py     # 사이트별 크롤러(뽐뿌/루리웹/펨코/아카라이브), 가격 추출
...
2. 봇이 10분마다 4개 커뮤니티 핫딜 게시판을 수집합니다.
...
- `/핫딜검색`
- `keyword`: 검색할 키워드(제목/검색어 모두 공백 제거 후 포함 여부로 검색)
- `limit`: 표시할 최대 게시글 수(선택, 기본 5 / 최대 10)
- `sort`: 정렬 기준(선택, `최신순` / `최저가순`, 기본 `최신순`)
- 최근 15일 범위를 기준으로 각 커뮤니티 페이지를 순회 수집해 검색
- 뽐뿌 결과는 상위 일부 검색 결과에 한해 로컬 `ollama`의 `gemma4`로 상품명/가격/상품 URL을 보강
